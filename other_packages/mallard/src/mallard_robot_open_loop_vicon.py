...
def thrust_ramp(current_time,rise_time,ss_time,max_thrust):

    slope = max_thrust/rise_time

    if current_time < rise_time:
        thrust = slope * current_time
    if current_time => rise_time and current_time < ss_time:
        thrust = max_thrust
...
def vicon_callback(msg):
    ...
    psi = tft.euler_from_quaternion([msg.pose.orientation.x,msg.pose.orientation.y,\
                                     msg.pose.orientation.z,msg.pose.orientation.w])[2] # returns yaw
...
def control_callback(event):
    ...
    if(joy_button_L1 == 1 and joy_button_R1 == 1):
        ...
        # Rotation matrix, R^-1:
        vx_body =  math.cos(psi)*x_vel + math.sin(psi)*y_vel
        vy_body = -math.sin(psi)*x_vel + math.cos(psi)*y_vel
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("/vicon_pose",PoseStamped, vicon_callback, param)
