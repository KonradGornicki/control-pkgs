...
def callback(data):
	...
	if flag == True:
		...
		if abs(data.pose.position.x - x_goal) < 0.02:
			...
			x_des = x0 + (abs(x_goal - x0) / (x_goal - x0)) * t_vlad * 0.05  # why -x0? in case it goes past the target, it can't go back
			...
			rospy.loginfo("t_vlad: %s, x0: %s", t_vlad, x0)

		if abs(data.pose.position.y - y_goal) < 0.02:
			twist.linear.y = -25
		else:
			y_des = y0 + abs(y_goal - y0) / (y_goal - y0) * t_vlad * 0.05
			twist.linear.x = (y_des - data.pose.position.y) * gy

		if abs(2 * math.atan2(data.pose.orientation.z, data.pose.orientation.w) - phi_goal) < 0.02:
			twist.angular.z = -25
		else:
			...
			if abs(phi_des - phi_goal) < 0.01:
				flag_phi = True
				#rospy.loginfo("FLAG PHI TRUE")
			if flag_phi:
				twist.angular.z = (phi_goal - current_phi) * gpsi
				#rospy.loginfo("Current Phi: %s, and Twist ang z: %s", current_phi, twist.angular.z)
			else:
				twist.angular.z = (phi_des - current_phi) * gpsi
				# rospy.loginfo("Des Phi: %s, and Phi goal: %s", phi_des, phi_goal)

		# twist.linear.x = -25
		# twist.linear.x = twist.linear.x * cos(psi) - twist.linear.y *sin(psi)
		twist.linear.y = -25
		twist.angular.z = -25
