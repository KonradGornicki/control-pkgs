...
# control parameters
# param = dict(vel=0.1, psivel=0.3, kp=0, kd=10, kp_psi=0, kd_psi=4,
#              lim=2, lim_psi=1, goal_tol=0.02, goal_tol_psi=0.1, nv=8)
...
param_obs = {'FK': 2,  # Force gain - PRIMARY tuning parameter
             'g_cuv': 0.3,  # gradient for force curve
             'f_lim': 5,  # force limit
             ...
             'r_min': 0.19,  # minimum radius where obstacle is considered (m)
             'r_max': 0.4,  # maximum radius where obstacle is considered (m)
...
def callback(data, paramf):
    ...
    psif_nav = kglocal.contpsi_fun(q_now, qdes, psivel, psiveldes, paramf['kp_psi'], paramf['kd_psi'],
                                   paramf['lim_psi'])
