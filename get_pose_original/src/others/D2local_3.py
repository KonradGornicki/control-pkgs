...
param = dict( vel=0.1, psi_vel=0.5, kp=2, kd=10, kppsi=0, lim=1, limpsi=1, nv=10)
# param = dict( vel=0.1, psi_vel=0.5, kp=2, kd=10, kppsi=1, lim=1, limpsi=1, nv=10)

tv = coll.deque([1e-5, 1.01e-5], maxlen=param['nv'])
xv = coll.deque([1e-5, 1.01e-5], maxlen=param['nv'])
yv = coll.deque([1e-5, 1.01e-5], maxlen=param['nv'])
psiv = coll.deque([1e-5, 1.01e-5], maxlen=param['nv'])

# this runs when new lidar data is published and it publishes on the twist topic
def callback(data, paramf):
    ...
    # build history vectors and find velocity
    tv.appendleft(t_now)
    ...
    xf_nav = contKG_fun(data.pose.position.x, xdes, xvel, xveldes, paramf['kp'], paramf['kd'], paramf['lim'])
    yf_nav = contKG_fun(data.pose.position.y, ydes, yvel, yveldes, paramf['kp'], paramf['kd'], paramf['lim'])
    psif_nav = contKGpsi_fun(q_now, qdes, paramf['kppsi'], paramf['limpsi'])
    ...
    twist.angular.z = -psif_nav             # minus is a fix to account for wrong direction on robot
...
def vel_fun(xyvec, tv):
    dv = [i-j for i, j in zip(xyvec[:-1], xyvec[1:])]
    dt = [i-j for i, j in zip(tv[:-1], tv[1:])]
...
def contKG_fun(xy, des, vel, veldes, kp, kd, lim):
    fp = (des - xy) * kp
    fd = (veldes - vel) * kd
    f_nav = fp + fd
    # rospy.loginfo("fp: %s, fd: %s", fp, fd)
    if abs(f_nav) > lim:
...
def contKGpsi_fun(q, des, kp, lim):
